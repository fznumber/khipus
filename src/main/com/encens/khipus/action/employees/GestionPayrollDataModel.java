...
/**
 * Data model for GestionPayroll
 *
 * @author
 * @version 2.26
 */

@Name("gestionPayrollDataModel")
@Scope(ScopeType.PAGE)
@Restrict("#{s:hasPermission('GESTIONPAYROLL','VIEW')}")
public class GestionPayrollDataModel extends QueryDataModel<Long, GestionPayroll> {
    @In(required = false)
    ...
    private static final String[] RESTRICTIONS = {
            "lower(gestionPayroll.gestionName) like concat('%', concat(lower(#{gestionPayrollDataModel.criteria.gestionName}), '%'))",
            "gestion.year = #{gestionPayrollDataModel.year}",
            "gestionPayroll.month = #{gestionPayrollDataModel.month}",
            "gestionPayroll.gestionPayrollType = #{gestionPayrollDataModel.gestionPayrollType}",
            "gestionPayroll.payrollGenerationCycle = #{gestionPayrollDataModel.payrollGenerationCycle}"
    };
    ...
    public String getEjbql() {
        return "select gestionPayroll from GestionPayroll gestionPayroll" +
                ...
                " where gestionPayroll.payrollGenerationCycle is null ";
    }
