 ...
 * @version 3.3
 */
public class MinProductItemControlWidgetSql implements SqlQuery {

    protected Log log = org.jboss.seam.log.Logging.getLog(this.getClass());

    private Integer businessUnitId;
    private SemaphoreState semaphoreState;

    public String getSql() {
        log.debug("SQL for semaphore state:" + semaphoreState);
    ...
    private String applyFilters() {
        ...
        if (SemaphoreState.GREEN.equals(semaphoreState)) {
            //filter as valid range of product item stock MIN
            filter += " AND INV.SALDO_UNI >= ART.STOCKMINIMO AND (ART.STOCKMAXIMO IS NULL OR INV.SALDO_UNI < ART.STOCKMAXIMO) \n";
        }else if (SemaphoreState.RED.equals(semaphoreState)) {
            //filter as out range of product item stock
            filter += " AND INV.SALDO_UNI < ART.STOCKMINIMO \n";
        } else {
            //return empty result
